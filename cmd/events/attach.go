...
var eventsAttachExamples = `
	...
	# attach to some-events events and output running messages filter by include regex (some*)
	kubetools events attach some-events -i some*

	# attach to some-events events and output running messages filter by exclude regex (not-some*)
