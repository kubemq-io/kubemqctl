...
func determineColor(podName string) (namespaceColor, podColor, containerColor *color.Color) {
	hash := fnv.New32()
	hash.Write([]byte(podName)) //nolint
...
func (t *Tail) Start(ctx context.Context, i v1.PodInterface) {
	t.namespaceColor, t.podColor, t.containerColor = determineColor(t.PodName)

	go func() {
		g := color.New(color.FgHiGreen, color.Bold).SprintFunc()
		p := t.podColor.SprintFunc()
		c := t.containerColor.SprintFunc()
		n := t.namespaceColor.SprintFunc()
		fmt.Fprintf(os.Stderr, "%s %s %s › %s\n", g("+"), n(t.Namespace), p(t.PodName), c(t.ContainerName))
...
func (t *Tail) Close() {
	...
	p := t.podColor.SprintFunc()
	fmt.Fprintf(os.Stderr, "%s %s %s\n", p(t.Namespace), p(t.PodName), p(t.ContainerName))
