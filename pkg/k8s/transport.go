...
func SetTransportWithService(ctx context.Context, cfg *config.Config) error {
	if !cfg.AutoIntegrated {
		return nil
	}
	utils.Print("connecting to kubernetes cluster... ")
	...
	stopCh := make(chan struct{})
	outCh, errOutCh := make(chan string, 1), make(chan string, 1)
	err = c.ForwardPortsToService(podNameSpace, "kubemq-cluster-grpc", []string{"50000"}, stopCh, outCh, errOutCh)
	if err != nil {
		return err
	}
	select {
